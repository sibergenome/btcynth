...
const minutelyPriceLastDay = "https://min-api.cryptocompare.com/data/v2/histominute?fsym=DOGE&tsym=USD&limit=1439";
...
const normalizeDecimal = (raw) => {
    ...
    const normalizedVals = vals.map( val => (val-min)/divider);
...
.get('/dogesynth/sample', (req, res) => {
    ...
    fetch(minutelyPriceLastDay)
        ...
        .then(json => {
            ...
            const pathname = path.resolve(__dirname, 'doge.wav');
